...
{
    CHECK_THROW_ERRNO(_sockfd >= 0, "socket() failed!");
}

Socket::Socket(int sock_fd) : _sockfd(sock_fd)
{
    CHECK_THROW_ERRNO(sock_fd != -1, "Invalid socket!");
}
...
Socket::Socket(Type sock_type, uint16_t local_port, bool reuse_addr) : _sockfd(socket(AF_INET, sock_type == Type::TCP ? SOCK_STREAM : SOCK_DGRAM, 0))
{
    ...
    if(reuse_addr)
    {
        CHECK_THROW_ERRNO(setsockopt(_sockfd, SOL_SOCKET, SO_REUSEADDR, (const char*)&reuse_addr, sizeof(int)) == 0, "setsockopt() failed");
    }
...
{
    CHECK_THROW_ERRNO(!setsockopt(_sockfd, level, optname, optval, optlen), "setsockopt failed!");
}

int Socket::get_sock_opt(int level, int optname, void* optval, socklen_t* optlen)
{
	int res;
    CHECK_THROW_ERRNO((res = getsockopt(_sockfd, level, optname, optval, optlen)) == 0, "getsockopt() failed");
    return res;
}
...
Socket::~Socket()
{
    ...
    (
		int sock = _sockfd;
        _sockfd = 0;
        CHECK_THROW_ERRNO(!close(sock), "close() failed");
    )
}
...
void Socket::bind_to_device(const string& iface_name)
{
    ...
    CHECK_THROW_ERRNO(!setsockopt(_sockfd, SOL_SOCKET, SO_BINDTODEVICE, (void *)&ifr, sizeof(ifr)), "failed to bind socket to interface %s", iface_name.c_str());
}
...
void Socket::set_receive_timeout(int ms)
{
    ...
    CHECK_THROW_ERRNO(setsockopt(_sockfd, SOL_SOCKET, SO_RCVTIMEO, (char *)&t, sizeof(t)) == 0, "setsockopt() failed");
}
...
void Socket::set_send_timeout(int ms)
{
    ...
    t.tv_sec = ms / 1000;
    t.tv_usec = (ms * 1000) % 1000000;
    CHECK_THROW_ERRNO(setsockopt(_sockfd, SOL_SOCKET, SO_SNDTIMEO, (char *)&t, sizeof(t)) == 0, "setsockopt() failed");
}
